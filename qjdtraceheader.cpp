...
bool QJDTraceHeader::convertFromData( const QByteArray& ba, QDataStream::ByteOrder bo,QJDDataStream::DataFormat df )
{
    if ( ba.size() != len() )
    {
...
bool QJDTraceHeader::convertToData( const QByteArray& ba, QDataStream::ByteOrder bo,QJDDataStream::DataFormat df )
{
    if ( ba.size() != len() )
    {
        qWarning(  "QJDTraceHeader: The size of bytearry is not equal to 240!"  );
...
void QJDTraceHeader::initialize()
{
    tracl=0;
    ...
    duse=1;         /* data use: 1 = production; 2 = test 035-036*/
    offset=0;       /* distance from source point to receiver
                       group (negative if opposite to direction
                       in which the line was shot) 037-040*/
    gelev=0;        /* receiver group elevation from sea level (above sea level is positive) 041-044*/
    selev=0;        /* source elevation from sea level (above sea level is positive) 045-048*/
    ...
    scalel=0;       /* scale factor for previous 7 entries with value plus or minus 10 to the
                       power 0, 1, 2, 3, or 4 (if positive, multiply, if negative divide) 069-070*/
    scalco=0;       /* scale factor for next 4 entries with value plus or minus 10 to the power 0, 1, 2, 3, or 4
                       (if positive, multiply, if negative divide) 071-072*/
    ...
    counit=1;       /* coordinate units code: for previous four entries
                      1 = length (meters or feet)
                      2 = seconds of arc (in this case, the
                      X values are longitude and the Y values are latitude, a positive value designates
                      the number of seconds east of Greenwich or north of the equator 089-090*/
...
QJDDataStream &operator<< ( QJDDataStream & ds, const QJDTraceHeader &s)
{
...
QJDDataStream &operator>> ( QJDDataStream & ds, QJDTraceHeader &s)
{
    ds>>s.tracl>>s.tracr>>s.fldr>>s.tracf>>s.ep>>s.cdp>>s.cdpt
            >>s.trid>>s.nvs>>s.nhs>>s.duse>>s.offset>>s.gelev>>s.selev
            >>s.sdepth>>s.gdel>>s.sdel>>s.swdep>>s.gwdep>>s.scalel
...
QTextStream &operator<< ( QTextStream & ts, const QJDTraceHeader &s)
{
    ts<<" tracl "<<s.tracl
            <<" tracr "<<s.tracr
            <<" fldr "<<s.fldr
